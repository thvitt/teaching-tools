...
def getargparser():
    ...
    p.add_argument(
        "-p", "--pattern", default="*.py", help="glob pattern which files to check"
    )
    p.add_argument(
        "-d",
        "--dendrogram",
        nargs="?",
        const="(show)",
        help="generate and optionally save a dendrogram",
    )
    p.add_argument(
        "-H",
        "--headings",
...
def load(files) -> pd.Series:
    return pd.Series(
        data=[
            load_unified_source(file, copy_unparseable=True)
...
def cluster(diffs: pd.DataFrame, threshold: float):
    ...
    flattened = pd.Series(
        index=diffs.index,
        data=sch.fcluster(clustering, threshold * max(dist_vect), "distance"),
    )
...
def dendrogram(clustering, labels, output, threshold):
    dend = sch.dendrogram(
        clustering, orientation="left", labels=labels, color_threshold=threshold
    )
    if output == "(show)":
        plt.show()
    else:
