...
parser.add_argument('-t', '--to', nargs='?', const='from',
...
template_vars = {v.value for v in pymustache.compiled(template).children if v.type_string == 'V'}
columns = set(reader.fieldnames)

if template_vars - columns:
    print("ERROR: The following template variables are not in the table:", ", ".join(template_vars - columns))
if columns - template_vars:
    print("INFO: The following table columns are not in the template:", ", ".join(columns - template_vars))
