...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="text()">
    <xsl:if test="normalize-space() != ''">
      ...
      <xsl:variable name="content" select="t:shorten($content_, 20, 20, 'end')"/>
      {generate-id(.)} [label="{$content}", {$text}]; {generate-id(..)} -&gt; {generate-id(.)};
    </xsl:if>
  </xsl:template>

  <xsl:template match="@*"> {generate-id(.)} [label="{name()}|{t:shorten(., 20, 20, 'middle')}", {$attribute}]; {generate-id(..)}
